...
public class Login_stepDefs {
    ...
    public void student_enters_valid_email_address() {
        login.txt_username.sendKeys(ConfigurationReader.getProperty("studentUsername1"));
    }

    @When("student enters valid password")
    public void student_enters_valid_password() {
        login.txt_password.sendKeys(ConfigurationReader.getProperty("studentPassword"));
    }
    ...
    public void student_should_see_library_title() {
        BrowserUtils.waitFor(4);
        Assert.assertEquals("Library", Driver.getDriver().getTitle());
    }


    @When("student enters invalid {string} address")
    public void student_enters_invalid_address(String emailAddress) {
        login.txt_username.sendKeys( (emailAddress.equals("empty") ? "" : emailAddress) );
    }

    @When("student enters invalid {string}")
    public void student_enters_invalid(String password) {
        login.txt_password.sendKeys( (password.equals("empty") ? "" : password) );
    }

    @Then("student should not be able to login")
    public void student_should_not_be_able_to_login() {
        String expectedTitle = "Login - Library";
        String actualTitle = Driver.getDriver().getTitle();
        Assert.assertEquals(expectedTitle, actualTitle);
    }
